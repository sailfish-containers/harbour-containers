...
However, if you had a version of `harbour-containers`, `lxc-templates-desktop` or `qxcompositor` installed from a source other than Chum, then you will need to uninstall them first because Chum will not overwrite older packages if they don't match vendor (presumably installed manually):

[source,bash]
----
devel-su
zypper remove lxc-templates-dekstop
zypper remove qxcompositor
zypper remove harbour-containers
----
...
=== How to use?
The GUI of `harbour-containers` should be relatively straightforward:
